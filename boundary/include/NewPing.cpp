...
/*STRICTLY FOR LOCAL TESTING EDITED COPY*/
...
NewPing::NewPing(unsigned int trigger_pin, unsigned int echo_pin, unsigned int max_cm_distance) {
#if DO_BITWISE == true
	...
	_triggerMode = (unsigned int *) portModeRegister(digitalPinToPort(trigger_pin)); // Get the port mode register for the trigger pin.
#else
...
unsigned long NewPing::ping_median(unsigned int it, unsigned int max_cm_distance) {
